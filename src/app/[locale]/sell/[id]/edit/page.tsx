...
import { SellForm, type SellFormInitial } from "@/components/sell/SellForm";
...
export default async function EditListingPage({
  ...
  const { data: prop } = await supabase
    .from("properties")
    .select("id, owner_id, title, description, type, area_sqm, rooms, bathrooms, floor, year_built, governorate, delegation, address, status")
    .eq("id", id)
    .single();
  if (!prop) notFound();
  if (prop.owner_id !== user!.id) {
    redirect({ href: "/sell", locale: locale as "ar" | "fr" | "en" });
  }
