-- ============================================================================
-- AUCTION CONFIG — sixth-offer (1/6 surenchère) becomes a per-auction SELLER
-- CHOICE, and auction FORMATS become admin-gated.
--
-- 1) New column auctions.sixth_offer_enabled (default FALSE). The legal 1/6
--    overbid window used to open automatically on EVERY won auction. Sellers
--    found it confusing, so it's now opt-in: the schedule form sets this flag.
--    tick_auctions' CLOSE branch honors it — enabled → open the 8-day window
--    (unchanged); disabled → award immediately with the 14-day final-payment
--    deadline (same end state the sixth-offer finalize would have produced
--    with no overbids).
--
-- 2) BEFORE-INSERT guard on auctions: 'english' is always allowed; 'dutch' and
--    'sealed' are only allowed when the admin enabled them in app_settings
--    ('auction_types' = {"dutch_enabled":bool,"sealed_enabled":bool}). So the
--    platform owner decides which formats sellers can create — the default is
--    English-only. Enforced server-side so it can't be bypassed by hitting
--    PostgREST directly; the schedule UI also hides disabled formats.
--
-- tick_auctions body is otherwise VERBATIM from 0124 (only the valid-winner
-- CLOSE branch changed). Idempotent (create or replace / add column if not
-- exists).
-- ============================================================================

alter table public.auctions
  add column if not exists sixth_offer_enabled boolean not null default false;

-- ── Auction-format availability guard ───────────────────────────────────────
...
declare
  v_types jsonb;
begin
  -- System relists (tick_auctions re-inserts an unsold lot) and direct-sale
  -- auto-creation preserve an already-valid format — never re-gate them, or a
  -- format disabled mid-flight would fail the whole cron transaction.
...
drop trigger if exists guard_auction_type_enabled on public.auctions;
create trigger guard_auction_type_enabled
  before insert on public.auctions
  for each row execute function public._guard_auction_type_enabled();

-- ── tick_auctions — sixth-offer window now gated on sixth_offer_enabled ──────
