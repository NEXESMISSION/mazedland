-- ============================================================================
-- NOTIFICATIONS (UX) — call registered bidders back when the auction opens.
--
-- A buyer who paid the caution on a SCHEDULED auction is told "vous pourrez
-- enchérir dès le démarrage" — but tick_auctions only notified the seller and
-- the watchlist when the lot went live. A depositor who hadn't also
-- watchlisted the lot was never called back, and the generic watcher
-- notification links to the auction page, not the bid room.
--
-- Fix, in the START phase:
--   1. seller notification — unchanged;
--   2. NEW: every user with an ACTIVE deposit (not released / not forfeited)
--      gets an 'auction_live' notification deep-linking to /auctions/<id>/bid
--      — they're cleared to bid, so drop them in the bid room directly;
--   3. watchers keep the generic /auctions/<id> notification, now EXCLUDING
--      depositors so nobody is notified twice (the bid-ready message wins).
--
-- Body is otherwise copied verbatim from 0078 (bounded batches, skip locked,
-- close + sixth-offer phases untouched). Idempotent (create or replace).
-- ============================================================================
...
declare
  ...
  -- Max rows handled per run, per phase. Steady state is far below this; the
  -- cap only bounds a catch-up run after a cron gap.
  v_batch     int := 500;
begin
  ...
  loop
    ...
    -- Registered bidders: their caution is locked on this lot, so they're
    -- cleared to bid the moment it opens — deep-link straight to the bid
    -- room. distinct guards against multiple active deposit rows.
    perform public.enqueue_notification(
      d.user_id,
      'auction_live',
    ...
    -- Watchers (minus depositors — the bid-ready message above wins).
    perform public.enqueue_notification(
      w.user_id,
      'auction_live',
      'Enchère en direct',
