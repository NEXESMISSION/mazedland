-- ============================================================================
-- BACKGROUND JOBS (Medium) — bound tick_auctions per run.
--
-- tick_auctions (restored in 0071) processes EVERY due row in three serial
-- cursor loops (start / close / sixth-offer-finalize) inside ONE transaction,
-- with no per-run cap. Under normal */1 cron that's fine, but after a cron
-- outage (or a large simultaneous-close wave) a single invocation would try to
-- close thousands of auctions in one long transaction — holding row locks,
-- enqueuing thousands of notifications, and risking a statement/transaction
-- timeout that rolls the WHOLE batch back (so nothing closes).
--
-- Fix: add a LIMIT to each FOR UPDATE SKIP LOCKED cursor so any single run does
-- bounded work and commits; the next cron tick drains the rest. SKIP LOCKED +
-- the bounded scan also means two overlapping runs never fight over the same
-- rows. TICK_BATCH is generous (500) — far above steady-state due-counts, so it
-- only ever bites during catch-up.
--
-- Body is otherwise copied verbatim from 0071 (notifications + auto-relist +
-- final_payment_due stamping). Only the three `limit` clauses are new.
