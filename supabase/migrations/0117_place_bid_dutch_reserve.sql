-- ============================================================================
-- AUCTION/MONEY (MED) — Dutch auctions must honor reserve_price.
--
-- The DB permits a reserve_price on a dutch lot, but place_bid's dutch branch
-- only checked dutch_price_drifted, then hammered status='ended_sold' at the
-- current descending price — so if dutch_floor_price < reserve_price the price
-- could descend below the seller's reserve and a buyer would win BELOW reserve
-- (the seller's stated minimum is silently ignored = money loss for the seller).
--
-- Fix: in the dutch branch, after the drift check, reject acceptance when the
-- current dutch price is below the reserve ('dutch_reserve_not_met'). Since the
-- dutch price only descends, once it crosses below reserve no valid acceptance
-- exists and tick_auctions closes the lot ended_unsold at expiry. The bid route
-- already maps the new code. Body is otherwise VERBATIM from 0091 (the only
-- change is the 3-line guard); english/sealed paths untouched.
-- ============================================================================
