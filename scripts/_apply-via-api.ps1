# Apply migrations 0066-0074 via the Supabase Management API (runs SQL as
# postgres using the CLI access token from the Windows vault). No DB password,
# no pooler IPv6. Preflights first; aborts cleanly if the account lacks SQL
# privileges so it never half-applies.
$ErrorActionPreference = 'Stop'
$ref = 'sajxoovrsoacfnytiijv'
$uri = "https://api.supabase.com/v1/projects/$ref/database/query"
$targets = '0066','0067','0068','0069','0070','0071','0072','0073','0074','0075'
...
"@
# Prefer an explicitly-provided token (SB_TOKEN); else read the CLI vault.
$token = $env:SB_TOKEN
if (-not $token) {
...
catch { Write-Output "PREFLIGHT_FAILED: $_"; Write-Output "=> This account lacks SQL privileges. Use Option A (SUPABASE_DB_PASSWORD)."; exit 3 }
...
foreach($v in $targets){
  $file = Get-ChildItem "supabase/migrations" -Filter "$($v)_*.sql" | Select-Object -First 1
  if(-not $file){ Write-Output "! $v : no file"; continue }
  ...
  $hist = "insert into supabase_migrations.schema_migrations (version,name,statements) values ('$v','$name','{}') on conflict (version) do nothing;"
